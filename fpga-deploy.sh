...
Examples:
  ...
  ./fpga-deploy.sh --target rp125_14 --ip rp-foo.local --bitstream ./ads1278.bit.bin
  ./fpga-deploy.sh --target rp125_14 --ip 169.254.97.245 --no-program
EOF
}
...
detect_default_bitstream() {
  local base_dir="$1"
  local impl_dir="$base_dir/rp_ads1278.runs/impl_1"
  local candidates=(
...
if [[ -z "$BITSTREAM_PATH" || ! -f "$BITSTREAM_PATH" ]]; then
  ...
  echo "Expected .bit.bin in rp_ads1278.runs/impl_1/" >&2
  exit 1
fi

if ! is_bit_bin_format "$BITSTREAM_PATH"; then
  impl_dir="$BUILD_DIR/rp_ads1278.runs/impl_1"
