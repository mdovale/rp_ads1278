...
- **Implemented (uncommitted):** Programmable LVCMOS square wave on `exp_p_io[5]` (package pin L16), MMIO register `MOD_DIV` @ `0x5C` on the DMA continuation branch, TCP protocol bump to `ads1278_v2` (64-byte messages, `mod_div` field), opcode `SET_MOD_DIV` (5), client **MOD freq** control in Hz.
...
|--------|------|--------|----------|
| `0x5C` | `MOD_DIV` | R/W | Half-period divider; `f_mod = 125 MHz / (2 × MOD_DIV)` |
...
- `fpga/rtl/ads1278_axi_slave.sv`: `mod_div_reg`, square-wave counter on `bus.ACLK`, `mod_o` output; read/write `0x5C` on the DMA continuation branch.
...
1. **New server v2 + old FPGA** — read @ `0x5C` returns garbage (`0xDEADBEEF`, `0xFFFFFFFF`, etc.).
...
**Fix:** Deploy **new bitstream**, reboot/reload PL, then client **Set MOD** at **10 Hz**, or `rpdevmem` write `0x5C` = `6250000`. Confirm with scope on DIO5_P (L16).
