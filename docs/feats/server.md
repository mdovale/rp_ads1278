...
- `START_LOCAL_LOG` bit `8` requests demod-rate logging for CH1-only captures. The server rejects that start with `ERROR` unless the current control snapshot has acquisition plus demod enabled (`CTRL & 0x6 == 0x6`).
...
4. The server writes rows for legacy samples, DMA samples, or expanded DMA bulk frames. With demod-rate CH1 logging requested, it writes only the first row, CH1 changes, or the elapsed demod frame interval.
...
- With CH1-only logging and `CTRL & 0x6 == 0x6`, enable demod-rate CSV and confirm row count is near the MOD rate; with acquisition-only control, confirm `START_LOCAL_LOG` returns `ERROR`.
