...
| `0x38` | `DMA_CTRL` | R/W | DMA enable/mode/IRQ control for Phase 5 bring-up |
| `0x3C` | `DMA_BASE_ADDR` | R/W | Physical DDR base for the DMA test buffer |
| `0x40` | `DMA_BUF_SIZE` | R/W | DMA buffer size in bytes |
| `0x44` | `DMA_STATUS` | R | DMA enabled/running/error status and write index |
...
| `0x58` | `DMA_IRQ_ACK` | W1C | Clear sticky DMA interrupt/status bits |
...
- a latest-sample MMIO debug/control path
- staged DMA bring-up registers for the separate DDR data path
