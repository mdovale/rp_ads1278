...
REMOTE_DIR="${REMOTE_DIR:-~/rp_ads1278}"
...
preflight_check() {
  ...
  [[ ! -f "$FPGA_DIR/source/cfg_${target}/ads1278.tcl" ]] && missing="${missing:+$missing }$FPGA_DIR/source/cfg_${target}/ads1278.tcl"
...
usage() {
  cat <<EOF
Usage: $(basename "$0") --target rp125_14 [OPTIONS]
...
Options:
  ...
  --remote-dir DIR      Repo directory on remote host [default: ~/rp_ads1278]
...
fetch_remote_bitstream() {
  ...
  work_dir="$(get_board_work_dir "$TARGET")"
  remote_impl_dir="$REMOTE_DIR/fpga/$work_dir/rp_ads1278.runs/impl_1"
  local_impl_dir="$SCRIPT_DIR/fpga/$work_dir/rp_ads1278.runs/impl_1"
  remote_impl_dir_escaped="${remote_impl_dir// /\\ }"
  mkdir -p "$local_impl_dir"
  if command -v rsync &>/dev/null; then
    rsync -az -e "ssh -p $REMOTE_PORT" --include="*.bit" --include="*.bit.bin" --exclude="*" \
...
rm -f design.bif
REMOTEEOF
  remote_tmp="/tmp/rp_ads1278_bootgen_$$.sh"
...
if [[ "$TARGET" != "rp125_14" ]]; then
  echo -e "${RED}Error: --target must be rp125_14.${NC}" >&2
  exit 1
fi

# Clean action does not require Vivado or FPGA sources
...
if [[ "$USE_REMOTE" == "1" ]]; then
  ...
  work_dir="$(get_board_work_dir "$TARGET")"
  impl_dir="$SCRIPT_DIR/fpga/$work_dir/rp_ads1278.runs/impl_1"
  ensure_ads1278_bitstream_names "$impl_dir" ""
else
  work_dir="$(get_board_work_dir "$TARGET")"
  impl_dir="$SCRIPT_DIR/fpga/$work_dir/rp_ads1278.runs/impl_1"
  bit_path="$(find_bitstream_path "$impl_dir" || true)"
  if [[ -n "$bit_path" ]]; then
    if [[ "$USE_DOCKER" == "1" ]]; then
      ...
      run_cmd+=(-v "$SCRIPT_DIR:/work" -w "/work/fpga/$work_dir/rp_ads1278.runs/impl_1" "$DOCKER_IMAGE")
      bit_name="$(basename "$bit_path")"
      out_name="${bit_name%.bit}.bit.bin"
